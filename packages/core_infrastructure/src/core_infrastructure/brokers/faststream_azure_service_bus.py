...
from faststream.azure_service_bus import ServiceBusBroker
...
class FastStreamAzureServiceBusBroker(MessageBrokerPort):
    def __init__(self, settings: BrokerConfig) -> None:
        self._settings = settings
        self._broker = ServiceBusBroker(self._settings.broker_url)

    async def publish(self, event: EventEnvelope) -> None:
        logger.info(
            "broker.faststream.azure_service_bus.publish",
            extra={
...
register_broker("faststream_azure_service_bus", lambda settings: FastStreamAzureServiceBusBroker(settings))
